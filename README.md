...
通过一个启动器 App，在启动 ChatGPT 时**只为它**注入代理环境变量（`HTTP_PROXY / HTTPS_PROXY / ALL_PROXY`），强制其所有请求走本地代理（默认 `http://127.0.0.1:10808`，可用 `CHATGPT_PROXY` 覆盖）：
...
```bash
CHATGPT_PROXY=http://127.0.0.1:7890 ~/Applications/"ChatGPT Proxy.app"/Contents/MacOS/launcher
```
...
- **设计并实现方案**：代理启动器（只为该 App 注入代理环境变量，无 TUN、无全局副作用、不烧代理流量）
- **编写文档并推送本仓库**
